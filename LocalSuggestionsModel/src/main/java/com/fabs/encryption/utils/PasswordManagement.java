package com.fabs.encryption.utils;
...
public class PasswordManagement
{
    ...
    public static final String PARTS_JOIN_KEY = ":";
    ...
    public static final int PASSWORD_HASH_INDEX = 4;
    ...
    private static String createHash(char[] password) throws CannotPerformOperationException {
        ...
        String parts = HASH_TYPE +
                PARTS_JOIN_KEY + PBKDF2_ITERATIONS +
                PARTS_JOIN_KEY + hashSize +
                PARTS_JOIN_KEY + toBase64(salt) +
                PARTS_JOIN_KEY + toBase64(hash);
        return parts;
    }
    ...
    private static boolean verifyPassword(char[] password, String correctHash)
        ...
        String[] params = correctHash.split(PARTS_JOIN_KEY);
        if (params.length != HASH_SECTIONS) {
            throw new InvalidHashException("Fields are missing from the password hash.");
        }

        // Currently, Java only supports HASH_TYPE.
        if (!params[HASH_ALGORITHM_INDEX].equals(HASH_TYPE)) {
        ...
        byte[] hash;
        try {
            hash = fromBase64(params[PASSWORD_HASH_INDEX]);
        } catch (IllegalArgumentException ex) {
    ...
    // cannot extract the hash of a password in an on-line system using a timing attack,
